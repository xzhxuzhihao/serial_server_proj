...
void *handle_connection(void* arg) {
	...
	*serial_fd = open(reg_dir, SERIAL_DIRECTION);
	...
	fd_serial_client_t->close_signal = 0;
    pthread_create(&read_thread, NULL, read_data_from_serial, fd_serial_client_t);
	
	serial_thread_signal_act.close_signal = &fd_serial_client_t->close_signal;
	serial_thread_signal_act.thread = &read_thread;
	write_data_to_serial(client_fd_, serial_fd);

    pthread_create(&wait_close_thread, NULL, serial_thread_close, &serial_thread_signal_act);
	pthread_join(wait_close_thread, NULL);
