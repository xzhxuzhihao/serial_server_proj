...
struct share_room communicate_share_room;
...
void serial_para_proc(serial_port_info *arg_serial, char *buffer, struct termios *tty){
	...
	pthread_mutex_lock(&lock_communicate_signal); // lock the communicate signal
	...
	for(int i=0; i<SERIAL_FILE_BUF_LENTH&&type<6; i++){
		if(buffer[i] == SERIAL_PARA_QUOTA && type == -1){
			i++;
			while(buffer[i] != SERIAL_PARA_QUOTA)
				i++;
			type++;
		printf("2communicate_signal = %d\n", communicate_signal);
		}else if(buffer[i] == SERIAL_PARA_QUOTA && type == 0){
			i++;
			int j=0;
			for(; j<6; j++, i++){
			...
			type += 1;
		printf("2communicate_signal = %d\n", communicate_signal);
		}else if(buffer[i] == SERIAL_PARA_QUOTA && type == 1){
			i++;
			int j=0;
			data_bit[j++] = buffer[i++];
			data_bit[j] = '\0';
			if(!strcmp(data_bit, "8"))
				...
				(*tty).c_cflag |= CS6;
			type += 1;
		}else if(buffer[i] == SERIAL_PARA_QUOTA && type == 2){
			i++;
			int j=0;
			...
			type++;
		}else if(buffer[i] == SERIAL_PARA_QUOTA && type == 3){
			i++;
			int j=0;
			stop_bit[j++] = buffer[i++];
			stop_bit[j] = '\0';
			if(!strcmp(stop_bit, "1"))
				...
				(*tty).c_cflag |= CSTOPB;
			type++;
		}else if(buffer[i] == SERIAL_PARA_QUOTA && type == 4){
			i++;
			int j=0;
			for(; j<4; j++, i++){
			...
			type++;
		}else if(buffer[i] == SERIAL_PARA_QUOTA && type == 5){
			i++;
			int j=0;
			for(; j<5; j++, i++){
...
unsigned char *str_to_num(char *serial_standard){
	unsigned char *send_data = (unsigned char *)malloc(sizeof(unsigned char)*5);;
	int sum = 0;
	int tmp = 1;
	int seq = 0;
	for(int i=0; i<32; i++){
...
void* resend_time_func(void *arg){
	...
	printf("ok17\n");
		...
		if(!time_signal->the_end){
			int resend_error = write(time_signal->fd, time_signal->serial_standard, time_signal->num);
			usleep(8800);
			for(int i=0; i<9; i++){
				printf("%d ", time_signal->serial_standard[i]);
				if(i == 8){
					printf("\n");
					printf("%d\n", resend_error);
				}
...
void* left_key_process_func(void* arg){
	char command[WAN_LAN_COMMAND_SIZE];
	snprintf(command, sizeof(command), WAN_TO_LAN_COMMAND);
	int key_error = system(command);
	JUDGE_ERROR_RETURN_NULL(key_error, "Serial_config: system command execute error. num 14");
}
void* right_key_process_func(void* arg){
	char command[WAN_LAN_COMMAND_SIZE];
	snprintf(command, sizeof(command), LAN_TO_WAN_COMMAND);
	int key_error = system(command);
...
unsigned char* str_to_num_ipaddr(char *ipaddr_char){
	unsigned char* ipaddr = (unsigned char*)malloc(sizeof(unsigned char)*9);
	int sum=0;
	int tmp=1;
	int num=0;
...
void* communicate_ip_addr_send(void *arg){
	...
	signal.serial_standard = ip_addr;
...
void *send_communicate_signal_func(void *arg){
	while(1){
		...
		if(communicate_signal == 1){
			...
			printf("3communicate_signal = %d\n", communicate_signal);
			communicate_signal = 0;
			break;
		} 
		printf("communicate_signal = %d\n", communicate_signal);
...
void *communicate_send_seiral_standard(void *arg){
	...
	while(1){
	printf("ok12\n");
		int communicate_pthread_error = pthread_create(&send_signal, NULL, send_communicate_signal_func, NULL);
		JUDGE_ERROR_RETURN_NULL(communicate_pthread_error, "Serial_config: pthread_create error. num 21");
		pthread_cond_wait(&cond, &lock_tmp);

	printf("ok21\n");
		usleep(100); // wait for modifying
		serial_standard_num = str_to_num(serial_standard);
		signal.serial_standard = serial_standard_num;
		communicate_send_is_success(&signal, SIG_PARA_FRAME);
	}
	pthread_mutex_unlock(&lock_tmp);
}
void* serial_config_api(void* arg){
	pthread_t thread_serial[16];
	...
	char *serial_standard = (char*)malloc(sizeof(char)*32);
	serial_config *serial_config_a = (serial_config *)malloc(sizeof(serial_config));
	memset(serial_standard, 0, 32);
	...
	for(int i=0; i<16; i++){
  	...
  	printf("ok3\n");
		...
		serial_config_a->serial_port[i].fd_w = inotify_init();
		JUDGE_ERROR_RETURN_NULL(serial_config_a->serial_port[i].fd_w, "Serial_config_api:inotify_init error. num 8");
		serial_config_a->serial_port[i].wd = inotify_add_watch(serial_config_a->serial_port[i].fd_w, serial_config_a->serial_port[i].watch_serial_name, IN_MODIFY);
		JUDGE_ERROR_RETURN_NULL(serial_config_a->serial_port[i].wd, "Serial_config_api:inotify_add_watch error. num 9");
