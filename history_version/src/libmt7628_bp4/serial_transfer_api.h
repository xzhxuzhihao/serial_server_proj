...
#define PRINTF_IP_ADDRESS() \
				printf("The ip address ");\
				for(int i=0; i<12; i++){\
					if(i%4 == 3)\
						printf("%d ", ip_addr[i+1]);\
					else\
						printf("%d.", ip_addr[i+1]);\
				}\
...
#define PRINTF_SERIAL_PARA() \
					printf("Serial port %d parameter is  %s %s %s %s %s\n", arg_serial->seqence, data_bit, parity_bit, stop_bit, flowcontrol, port_type);
